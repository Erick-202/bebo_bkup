...
#   --->    ASI JALA pero no se centra angularmente
...
global blue_x, blue_y, blue_angle
blue_x = 0
blue_y = 0
blue_angle = 90


def blue_angle_callback(msg):
    global blue_angle
    blue_angle = float(msg.data)
...
def camaraCallback(msg):
    ...
    if sistema_armado == 1:
        ...
        if blue_x != 0 and blue_y != 0:
            ...
            print "ANgulo ", str(blue_angle)
            if float(blue_angle) < 0:
                cmd_vel.angular.z = -ang_z.calcular(90, -float( blue_angle))  # -88   ->  88  -> 90 , como es negativo, girar a la derecha o sea - angular.z
            else:
                cmd_vel.angular.z = ang_z.calcular(90, float(blue_angle))

            print cmd_vel.angular.z

            cmd_vel.linear.x = pos_x.calcular(2 * cam_h // 3, blue_y)
            cmd_vel.linear.y = pos_y.calcular(cam_w // 2, blue_x)
            cmd_vel.linear.z = 0
            cmd_vel.angular.z = ang_z.calcular(90, blue_angle)
...
def bebop():
    ...
    h_sub_py = rospy.Subscriber("blue_x", String, blue_x_callback)
    h_sub_py = rospy.Subscriber("blue_y", String, blue_y_callback)
    h_sub_py = rospy.Subscriber("blue_angle", String, blue_angle_callback)
