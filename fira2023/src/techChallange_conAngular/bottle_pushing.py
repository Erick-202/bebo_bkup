...
red_high1 = np.array([ 7, 255, 255], np.uint8)

red_low2 = np.array([255, 100, 20], np.uint8)
...
def image_callback(data):
   ...
   # eroded_Red= cv2.erode(maskRed, kernel, iterations=2)
    ...
    #dilate = cv2.dilate(eroded_Red,kernel,iterations=4)

    dilate = cv2.dilate(maskRed,kernel,iterations=4)

    _, contornos, _ = cv2.findContours(dilate, cv2.RETR_EXTERNAL,
    ...
    for c in contornos:
        ...
        if area >= areaMin and area <= areaMax:
            ...
            pub = rospy.Publisher('bottle_area', String, queue_size=2)
    ...
    pub = rospy.Publisher('bottle_x', String, queue_size=2)
    pub.publish(str(x))

    pub = rospy.Publisher('bottle_z', String, queue_size=2)
    pub.publish(str(y))
...
def receive_message():
    ...
    print ('receive msg')
    rospy.init_node('bottle_sub_py', anonymous=True)
